...
Schema get_schema(GraphHandle graph) {
  ...
  nocc::graph::SchemaImpl* schema = new nocc::graph::SchemaImpl();
  schema->property_id_map = ctx->graph_store->get_schema().property_id_map;
  schema->label_id_map = ctx->graph_store->get_schema().label_id_map;
  schema->dtype_map = ctx->graph_store->get_schema().dtype_map;
  schema->vlabel2prop_offset = ctx->graph_store->get_schema().vlabel2prop_offset;
  return schema;
}
...
void free_schema(Schema schema) {
    ...
    LOG(INFO) << "enter " << __FUNCTION__;
  #endif
  nocc::graph::SchemaImpl* schema_ptr = reinterpret_cast<nocc::graph::SchemaImpl*>(schema);
...
int get_property_id(Schema schema, const char* name, PropertyId* out) {
    ...
    LOG(INFO) << "enter " << __FUNCTION__;
  #endif
  SchemaImpl* schema_impl = (SchemaImpl*)schema;
  std::string query(name, name + strlen(name));
  auto iter = schema_impl->property_id_map.find(query);
  if (iter == schema_impl->property_id_map.end()) {
    return -1;
  }
  else {
    *out = iter->second;
...
int get_property_type(Schema schema, LabelId label, PropertyId id,
  ...
  #endif
  SchemaImpl* schema_impl = (SchemaImpl*)schema;
  auto iter = schema_impl->dtype_map.find(std::make_pair(label, id));
  if (iter == schema_impl->dtype_map.end()) {
    return -1;
  }
  else {
    *out = static_cast<PropertyType>(iter->second);
    return 0;
  }
}
...
int get_property_name(Schema schema, PropertyId id, const char** out) {
  #ifndef NDEBUG
    LOG(INFO) << "enter " << __FUNCTION__ << " property id = " << id;
  #endif
  SchemaImpl* schema_impl = (SchemaImpl*)schema;
  auto mp = schema_impl->property_id_map;
  for (auto iter = mp.begin(); iter != mp.end(); iter++) {
    //LOG(INFO) << " get property name = " << iter->first;
    if (iter->second == id) {
      *out = const_cast<char *>(iter->first.c_str());
      //if (id == 16) LOG(INFO) << "property name = "<<iter->first << " result = " << *out;
      return 0;
    }
  }
  return -1;
...
int get_label_id(Schema schema, const char* name, LabelId* out) {
  ...
  #endif
  SchemaImpl* schema_impl = (SchemaImpl*)schema;
  auto label_id_map = schema_impl->label_id_map;
  std::string query(name, name + strlen(name));
  auto iter = label_id_map.find(query);
  if (iter == label_id_map.end()) {
    return -1;
  }
  else {
    *out = iter->second;
...
int get_label_name(Schema schema, LabelId label, const char** out) {
  ...
  SchemaImpl* schema_impl = (SchemaImpl*)schema;
  auto label_id_map = schema_impl->label_id_map;
  for (auto iter = label_id_map.begin(); iter != label_id_map.end(); iter++) {

...
struct GraphCtx {
  ...
  inline GraphCtx(graph::GraphStore *g, uint64_t v, uint64_t b, uint64_t e)
    : graph(g), ver(v), off_begin(b), off_end(e), cnt(0), walk_cnt(0),
      mem_ms(10, 0.0) { }
};
...
class ChQueryWorker : public ChMixin, public QueryWorker {
 ...
 protected:
  ...
  // Graph HTAP micro
  DECLARE_MICRO(29, flex_prop_read);   // flexible property storage: read thpt
  DECLARE_MICRO(30, flex_prop_write);  // flexible property storage: read thpt

...
class LogCleaner {
 public:
  ...
  virtual void balance_index() { assert(false); }

  virtual void prepare_balance(int num_workers) { assert(false); }
  virtual void parallel_balance(int worker_id) { assert(false); }
  virtual void end_balance() { assert(false); }

  virtual void balance_index(const std::vector<int> &threads) { assert(false); }

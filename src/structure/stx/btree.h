...
// for count and btree verification
#define BTREE_COUNT 0
...
#include <cassert>
/// Assertion only if BTREE_COUNT == 1 is defined. This is not used in verify().
...
template <typename T>
static inline T fetch_and_add(T *ptr, T value) {
  return __sync_fetch_and_add(ptr, value);
}

inline static void bind_to_core_raw(int cpu_id) {
  cpu_set_t mask;
  CPU_ZERO(&mask);
  CPU_SET(cpu_id, &mask);
...
public:
    ...
    /// Lazy buffer parameter
    static const bool lazy = traits::lazy;
    
    /// Lazy buffer parameter
    static const int lazyslotmax = traits::lazyslots;
...
private:
    ...
    struct node
    {
        ...
        inline node(const unsigned short l, const unsigned short s = 0)
            : level(l), slotuse(s), parent_slot(0), parent(nullptr)
        { }
        ...
        inline void set_parent(inner_node *p, unsigned short s) {
            parent = p;
            parent_slot = s;
        }
    };
    ...
    typedef std::vector<keynode_t> keynode_vec_t; 
    ...
    struct inner_node : public node
    {
        ...
        /// #keys = `slotuse`, #children = `slotuse` + 1

        /// Keys of children or data pointers
        key_type slotkey[innerslotmax + 1];  // XXX: the last one is special
        ...
        static inline int num_inner(int num_children) {
            return (num_children + (innerslotmax + 1) - 1) / (innerslotmax + 1);
        }
    };
    ...
    struct leaf_node : public node
    {
        ...
        /// Define the structure for lazy insert buffer
        struct lazy_buffer_t {
            // pair_type pairs[lazy ? lazyslotmax : 1];
            std::atomic<int> slot_use;
            volatile int pairs_sz;
            std::vector<pair_type> pairs;

            lazy_buffer_t() : 
              slot_use(0), pairs_sz(lazy? lazyslotmax : 1), pairs(pairs_sz)
            { }
        };
        ...
        /// lazy insert buffer
        lazy_buffer_t lazy_buffer;

        /// Set variables to initial values
        inline leaf_node()
            : node(0), seq(0), prevleaf(NULL), nextleaf(NULL)
        { }

        /// Construction during restore from node top
        inline leaf_node(const node& top)
            : node(top.level, top.slotuse), seq(0), prevleaf(NULL), nextleaf(NULL)
        { }
        ...
        inline void set_slot(unsigned short slot,
        ...
        inline void lazy_clear() {
            lazy_buffer.slot_use = 0;
        }

        inline void lazy_sort() {
            // TODO: use function like `key_less`...?
            std::sort(&lazy_buffer.pairs[0], 
                      &lazy_buffer.pairs[lazy_buffer.slot_use]); 
        }

        inline bool is_lazy_full() {
            BTREE_ASSERT(false);
            return (lazy_buffer.slot_use == lazyslotmax);
        }
...
public:
    ...
    public:
        ...
        inline const uint64_t *get_seq_ptr() const {
          return currnode? &currnode->seq : nullptr;
        }
    ...
    public:
        ...
        inline const_iterator(const typename btree::leaf_node* l, unsigned short s)
            : currnode(l), currslot(s), currseq(currnode->seq)
        { }

        /// Copy-constructor from a mutable iterator
        inline const_iterator(const iterator& it) // NOLINT
            : currnode(it.currnode), currslot(it.currslot), currseq(currnode->seq)
        { }
        ...
        {
            if (currnode->seq != currseq) {
                while (currnode) {
                    int k = 0;
                    int num = currnode->slotuse;
                    while (k < num && currkey >= currnode->slotkey[currslot]) {
                        ++k;
                    }
                    if (k == num) {
                        if (currnode->nextleaf == nullptr) {
                            // this is end()
                            currslot = currnode->slotuse;
                            return *this;
                        } else {
...
public:
    ...
    struct tree_stats
    {
        ...
        // XXX[SSJ]: leaves and innernodes are replaced with level_nodes
        ...
        std::vector<size_type>      level_nodes;
        ...
        {
            return static_cast<double>(itemcount) / (level_nodes[0] * leafslots);
        }
    };
...
private:
    ...
    volatile int m_dirty_sz;
    std::vector<std::pair<leaf_node *, key_type>> m_dirty_leaf;
    std::atomic<int> m_dirty_num;
    ...
    struct BTask {
        ...
        key_type begin;
        key_type end;   // (begin, end]
        int max_level;
        std::vector<keynode_vec_t> split_vec;


        BTask(int t, key_type b, key_type e, int l) 
          : BTask(t, true, true, b, e, l) { }

        BTask(int t, bool fb, bool fe, key_type b, key_type e, int l)
          : tid(t), fbegin(fb), fend(fe), begin(b), end(e), max_level(l),
            complete(false) { }
    };

    volatile bool m_during_balance;
    std::vector<BTask> m_tasks;
...
public:
    ...
    explicit inline btree(const allocator_type& alloc = allocator_type())
        : m_root(NULL), m_headleaf(NULL), m_tailleaf(NULL), m_allocator(alloc),
          m_dirty_sz(lazy? 1024 : 0), m_dirty_leaf(m_dirty_sz), m_dirty_num(0),
          m_during_balance(false)
    { 
    }
...
public:
    ...
    inline void prepare_leaf_node(void **hint_leaf) {
        leaf_node *n = new (leaf_node_allocator().allocate(1))
                       leaf_node();
        *hint_leaf = reinterpret_cast<void *> (n);
    }
...
private:
    ...
    inline leaf_node * allocate_leaf()
    {
        ...
        BTREE_ASSERT(m_stats.level_nodes.size() >= 1);
        fetch_and_add<size_type>(&m_stats.level_nodes[0], 1);
        // fetch_and_add<size_type>(&m_stats.leaves, 1);
        return n;
    }
    
    inline leaf_node * allocate_leaf(leaf_node **hint_leaf)
    {
        leaf_node *n = nullptr; 
        if (hint_leaf && *hint_leaf) {
            n = *hint_leaf;
            *hint_leaf = nullptr;
        } else {
            n = new (leaf_node_allocator().allocate(1)) leaf_node();
        }
        // leaf_node *n = new leaf_node();
        BTREE_ASSERT(m_stats.level_nodes.size() >= 1);
        fetch_and_add<size_type>(&m_stats.level_nodes[0], 1);
    ...
    inline inner_node * allocate_inner(unsigned short level)
    {
        inner_node* n = new (inner_node_allocator().allocate(1))
                        inner_node(level);
        // inner_node *n = new inner_node(level);
        BTREE_ASSERT(m_stats.level_nodes.size() > level);
        fetch_and_add<size_type>(&m_stats.level_nodes[level], 1);
        // fetch_and_add<size_type>(&m_stats.innernodes, 1);
        return n;
    }

    inline void add_level(unsigned short level) {
        BTREE_ASSERT(m_stats.level_nodes.size() == level);
        m_stats.level_nodes.push_back(0);
    }
...
public:
    ...
    iterator find(const key_type& key)
    {
        ...
        {
            const inner_node* inner = static_cast<const inner_node*>(n);
#if BASELINE == 0  // find_baseline
            int slot = find_lower(inner, key);
#else
            int slot = find_lower_b(inner, key);
#endif
    ...
    inline data_type get(const key_type &key) {
        const node* n = m_root;
        if (!n) return data_type();
...
public:
    ...
    inline void insert_lazy(const pair_type& x) {
        insert_start_lazy(x.first, x.second);
    }

    inline void insert_lazy(const key_type &k, const data_type &d) {
        insert_start_lazy(k, d);
    }

    // single thread balance
    inline void balance() {
        // printf("m_dirty_num: %d\n", m_dirty_num.load());
        m_during_balance = true;
        for (int i = 0; i < m_dirty_num; ++i) {
            balance_leaf(m_dirty_leaf[i].first);
        }
        m_dirty_num = 0;
        m_during_balance = false;
    }
    ...
    inline void prepare_balance(int num_worker) {
        partition_task(num_worker, m_tasks);
        m_during_balance = true; 
    }

    inline void balance(int worker_id) {
        while (!m_during_balance) ;
        BTask &task = m_tasks[worker_id];
        paral_balance(task);
        task.complete = true; 
    }

    inline void end_balance() {
        for (const BTask &task : m_tasks) {
            while (!task.complete) ;
        }

        for (BTask &task : m_tasks) {
            for (keynode_vec_t &splits : task.split_vec) {
                // TODO: update inner nodes
                if (splits.size() == 0) continue;
                inner_node *parent = splits[0].n->parent;
                int parent_slot = splits[0].n->parent_slot;

                while (parent != nullptr) {
                    if (splits.size() == 0) break;
                    if (splits.size() == 1) {
                        parent->slotkey[parent_slot] = splits[0].k;
                        break;
                    }

                    balance_inner(parent, parent_slot, splits);
    ...
    inline void balance(const std::vector<int> &cpus) {
        int num_threads = cpus.size();
        ...
        BTREE_ASSERT(num_threads > 1);
        
        std::vector<BTask> tasks;
        partition_task(cpus, tasks);
        BTREE_ASSERT(tasks.size() > 1);

        m_during_balance = true;
        std::vector<std::thread> threads;
        for (BTask &task : tasks) {
            threads.emplace_back([&task, this] {
               bind_to_core_raw(task.tid); 
               paral_balance(task);
            });
        }
    ...
    inline void paral_balance(BTask &task) {
        // assert(m_dirty_num == 0); // to show if balance or not
        for (int i = 0; i < m_dirty_num; ++i) {
            key_type key = m_dirty_leaf[i].second;
            bool cond1 = !task.fbegin || key_greater(key, task.begin);
            bool cond2 = !task.fend || key_lessequal(key, task.end);
            if (cond1 && cond2) {
                // TODO: balance leaf
                leaf_node *leaf = m_dirty_leaf[i].first;
                keynode_vec_t splits;
                balance_leaf(leaf, task.max_level, splits);
                task.split_vec.emplace_back(std::move(splits));
            }
        } 
    }

    // collect keys in inner nodes whose level >= bound
    // reqiurement: bound >= 1 && bound <= m_root->level
    void walk_tree(inner_node *n, int bound, std::vector<key_type> &keys) {
        if (n->level == bound) {
            for (int i = 0; i < n->slotuse; ++i) {
              keys.push_back(n->slotkey[i]);
            }
            return;
        }
        ...
        for (int i = 0; i < n->slotuse; ++i) {
            child = n->childid[i];
            BTREE_ASSERT(!child->isleafnode());
            walk_tree(static_cast<inner_node *>(child), bound, keys);
            keys.push_back(n->slotkey[i]);
        }
        child = n->childid[n->slotuse];
        walk_tree(static_cast<inner_node *>(child), bound, keys);
    }

    void partition_task(const std::vector<int> tids, std::vector<BTask> &tasks) {
        BTREE_ASSERT(!m_root->isleafnode());

        int num_worker = tids.size();
        int max_level = -1;
        int total_nodes = 0;
        const std::vector<size_type> &nodes = m_stats.level_nodes;
        for (int i = nodes.size() - 1; i >= 0; --i) {
            if ((int) nodes[i] >= num_worker) {
                max_level = i;
                total_nodes = nodes[i];
                break;
            }
        ...
        BTREE_ASSERT(max_level < (int) nodes.size() - 1);

        std::vector<key_type> keys;
        assert(total_nodes >= 1);
        keys.reserve(total_nodes - 1);
        walk_tree(static_cast<inner_node *>(m_root), max_level + 1, keys);
        BTREE_ASSERT((int) keys.size() == total_nodes - 1);
        
        tasks.reserve(num_worker);
        int start_n = 0;
        for (int i = 0; i < num_worker; ++i) {
            int num_nodes = total_nodes / (num_worker - i);
            total_nodes -= num_nodes;
            key_type begin_key = (i == 0)? key_type() : keys[start_n - 1];
            key_type end_key = (i == num_worker - 1)? 
                               key_type() : keys[start_n + num_nodes - 1];
            tasks.emplace_back(tids[i], begin_key, end_key, max_level);
            start_n += num_nodes;
        }
        tasks[0].fbegin = false;
        tasks[num_worker - 1].fend = false;
    }

    void partition_task(int num_worker, std::vector<BTask> &tasks) {
        assert(!m_root->isleafnode());
        assert(num_worker > 1);
        ...
        if (max_level == -1) {
            assert(nodes[0] <= num_worker);
            num_worker = nodes[0];
            total_nodes = nodes[0];
            max_level = 0;
        }

        assert(max_level < (int) nodes.size() - 1);

        std::vector<key_type> keys;
        keys.reserve(total_nodes - 1);
        walk_tree(static_cast<inner_node *>(m_root), max_level + 1, keys);
        assert((int) keys.size() == total_nodes - 1);
        ...
        for (int i = 0; i < num_worker; ++i) {
            ...
            key_type end_key = (i == num_worker - 1)? 
                               key_type() : keys[start_n + num_nodes - 1];
            tasks.emplace_back(i, begin_key, end_key, max_level);
            start_n += num_nodes;
        }
        tasks[0].fbegin = false;
        tasks[num_worker - 1].fend = false;
...
#else
        return insert_start_baseline(key, data);
#endif
    }
    ...
    {
#if BASELINE == 0  // find_baseline
        return insert_start(key, data, reinterpret_cast<leaf_node **>(hint_leaf));
#else
        return insert_start_baseline(key, data);
#endif
    }
...
private:
    // *** our baseline
    inline int find_lower_b(const leaf_node *n, const key_type &key) const {
      int lo = 0;
      while (lo < n->slotuse && key_less(n->slotkey[lo], key)) ++lo;
    ...
    inline int find_lower_b(const inner_node *n, const key_type &key) const {
      int lo = 0;
      // XXX: less or equal
  ...
  insert_leaf_b(leaf_node *leaf, const key_type& key, const data_type& value,
                key_type* splitkey, node** splitnode) {

    leaf_node *new_sibling = NULL;
    int k = find_lower_b(leaf, key);

    if((k < leaf->slotuse) && key_equal(leaf->slotkey[k], key)) {
      leaf->slotdata[k] = value;
      return std::pair<iterator, bool>(iterator(leaf, k), false);
    }


    leaf_node *toInsert = leaf;
    if (leaf->isfull()) {
      new_sibling = allocate_leaf();

      int threshold= (leafslotmax + 1) / 2;
      new_sibling->slotuse= leaf->slotuse - threshold;
      for(int j = 0; j < new_sibling->slotuse; ++j) {
        new_sibling->slotkey[j]= leaf->slotkey[threshold+j];
        new_sibling->slotdata[j]= leaf->slotdata[threshold+j];
      }
      leaf->slotuse= threshold;
      ...
      if (leaf->nextleaf != NULL) { 
        leaf->nextleaf->prevleaf = new_sibling;
      } else {
        m_tailleaf = new_sibling;
      }
      new_sibling->nextleaf = leaf->nextleaf;
      new_sibling->prevleaf = leaf;
      leaf->nextleaf = new_sibling;
      new_sibling->seq = 0;
    }

    for (int j=toInsert->slotuse; j>k; j--) {
      toInsert->slotkey[j] = toInsert->slotkey[j-1];
      toInsert->slotdata[j] = toInsert->slotdata[j-1];
    }

    toInsert->slotuse = toInsert->slotuse + 1;
    toInsert->slotkey[k] = key;
    
    toInsert->slotdata[k] = value;

    leaf->seq = leaf->seq + 1;

    *splitnode = new_sibling;
    if (new_sibling)
      *splitkey = new_sibling->slotkey[0];

    return std::pair<iterator, bool>(iterator(toInsert, k), true);

  }
  
  inline std::pair<iterator, bool> 
  insert_inner_b(inner_node *inner, const key_type& key, const data_type& value,
                key_type* splitkey, node** splitnode) {
    std::pair<iterator, bool> r;
    unsigned k = 0;
    key_type upKey;
    inner_node *new_sibling = NULL;

    k = find_lower_b(inner, key);
    node *child = inner->childid[k];


    if (inner->level == 1) {
      r = insert_leaf_b(static_cast<leaf_node*>(child),
                                            key, value, splitkey, splitnode);
      leaf_node *new_leaf = static_cast<leaf_node*>(*splitnode);

      if (new_leaf != NULL) {
        inner_node *toInsert = inner;
      
        if (inner->isfull()) {
     
          new_sibling = allocate_inner(inner->level);
      
          unsigned treshold= (innerslotmax+1) / 2;
          new_sibling->slotuse = inner->slotuse - treshold;
    
          for(unsigned i=0; i < new_sibling->slotuse; ++i) {
            new_sibling->slotkey[i]= inner->slotkey[treshold+i];
            new_sibling->childid[i]= inner->childid[treshold+i];
          }
    
          new_sibling->childid[new_sibling->slotuse] = inner->childid[inner->slotuse];
          inner->slotuse= treshold-1;
    
          upKey = inner->slotkey[treshold-1];
     
          // XXX: >=  
          if (new_leaf->slotkey[0] >= upKey) {
              assert(k >= treshold);
              toInsert = new_sibling;
              k -= treshold;
          }
          new_sibling->slotkey[innerslotmax-1] = upKey;
        ...
        for (int i=toInsert->slotuse; i>k; i--) {
          toInsert->slotkey[i] = toInsert->slotkey[i-1];
          toInsert->childid[i+1] = toInsert->childid[i];
        }
        toInsert->slotuse++;
        toInsert->slotkey[k] = new_leaf->slotkey[0];
      
        toInsert->childid[k+1] = new_leaf;
      }
    }
    else {
      r = insert_inner_b(static_cast<inner_node*>(child),
                                            key, value, splitkey, splitnode);
      inner_node *new_inner = static_cast<inner_node*>(*splitnode);
      bool s = true;
      if (new_inner != NULL) {
        inner_node *toInsert = inner;
        inner_node *child_sibling = new_inner;
        ...
        if (inner->isfull()) {
          ...
          new_sibling->childid[new_sibling->slotuse]=
            inner->childid[inner->slotuse];
        ...
        toInsert->slotuse++;
        toInsert->slotkey[k] = reinterpret_cast<inner_node*>(child_sibling)->slotkey[innerslotmax-1];
        toInsert->childid[k+1] = child_sibling;
      }

    }

    if (inner == m_root && new_sibling != NULL) {
      inner_node *new_root = allocate_inner(inner->level + 1);
      add_level(inner->level + 1);
      new_root->slotuse = 1;
      new_root->slotkey[0]= upKey;
      new_root->childid[0] = m_root;
      new_root->childid[1] = new_sibling;
      m_root = new_root;
    }
    return r;
  }
    ...
    inline std::pair<iterator, bool>
    insert_start_baseline(const key_type& key, const data_type& value) {
      ...
      if (m_root->isleafnode()) {
        r = insert_leaf_b(static_cast<leaf_node*>(m_root),
                                            key, value, &newkey, &newchild);
        if (newchild != NULL) {
          inner_node *inner = allocate_inner(m_root->level + 1);
          inner->slotuse = 1;
          inner->slotkey[0] = newkey;
          inner->childid[0] = m_root;
        ...
        r = insert_inner_b(static_cast<inner_node*>(m_root),
                                            key, value, &newkey, &newchild);
      }
...
private:
    ...
    inline void split_leaf_node(leaf_node* leaf, key_type* _newkey, node** _newleaf, leaf_node **hint_leaf)
    {
        BTREE_ASSERT(leaf->isfull());

        // unsigned int mid = (leaf->slotuse >> 1);
        unsigned int mid = (leaf->slotuse + 1) / 2;

        BTREE_PRINT("btree::split_leaf_node on " << leaf);

        leaf_node* newleaf = allocate_leaf(hint_leaf);
        ...
        data_copy(leaf->slotdata + mid, leaf->slotdata + leaf->slotuse,
                  newleaf->slotdata);
#else
        for (int j = 0; j < newleaf->slotuse; ++j) {
            newleaf->slotkey[j] = leaf->slotkey[mid + j];
            if (!used_as_set)
                newleaf->slotdata[j] = leaf->slotdata[mid + j];
        }
#endif
    ...
    inline void split_inner_node(inner_node* inner, key_type* _newkey, node** _newinner, unsigned int addslot)
    {
        ...
        for (int i = 0; i < newinner->slotuse; ++i) {
            newinner->slotkey[i] = inner->slotkey[mid + 1 + i];
            newinner->childid[i] = inner->childid[mid + 1 + i];
        }
        ...
        for (int i = 0; i < newinner->slotuse + 1; ++i) {
            newinner->childid[i]->set_parent(newinner, i);
        }
                                           ...
                                           leaf_node **hint_leaf = nullptr)
    {
        ...
        std::pair<iterator, bool> r = insert_descend(m_root, key, value, &newkey, &newchild, hint_leaf);

        if (newchild)
        {
            add_level(m_root->level + 1);
            ...
            // set parent
            m_root->set_parent(newroot, 0);
            newchild->set_parent(newroot, 1);
...
#if BTREE_COUNT == 1
        // increment itemcount if the item was inserted
        if (r.second) fetch_and_add<size_type>(&m_stats.itemcount, 1);
#endif
    ...
    */
                                             ...
                                             leaf_node **hint_leaf)
    {
        if (!n->isleafnode())
        {
            ...
            if (newchild)
            {
                ...
                std::copy_backward(inner->childid + slot + 1, inner->childid + inner->slotuse + 1,
                                   inner->childid + inner->slotuse + 2);
#else
                for (int i = inner->slotuse; i > slot; --i) {
                ...
                for (int i = slot + 1; i < inner->slotuse + 2; ++i) {
                    inner->childid[i]->set_parent(inner, i);
                }
        ...
        else // n->isleafnode() == true
        {
            ...
            if (leaf->isfull())
            {
                split_leaf_node(leaf, splitkey, splitnode, hint_leaf);
...
#else
            for (int j = leaf->slotuse; j > slot; --j) {
                leaf->slotkey[j] = leaf->slotkey[j - 1];
                if (!used_as_set)
                    leaf->slotdata[j] = leaf->slotdata[j - 1];
            }
#endif
    ...
    inline void
    insert_start_lazy(const key_type& key, const data_type& value) {
        BTREE_ASSERT(m_root);
        ...
        // end = ::now();
        // m_stats.time[0] += end - begin;

        leaf_node* leaf = static_cast<leaf_node*>(n);
        lazy_insert(leaf, key, value);

#if 0
        if(leaf->is_lazy_full()) {
            begin = ::now();
            balance_leaf(leaf);
            m_stats.itemcount += lazyslotmax;
            end = ::now();
            m_stats.time[0] += end - begin;
        }
#endif
    ...
    void balance_leaf(leaf_node *leaf) {
        keynode_vec_t splits;
        int num_lazy = leaf->lazy_buffer.slot_use;
        update_lazy_buffer(leaf, splits);

        inner_node* parent = leaf->parent;
        int parent_slot = leaf->parent_slot;
...
#if BTREE_COUNT == 1
        // increment itemcount if the item was inserted
        fetch_and_add<size_type>(&m_stats.itemcount, num_lazy);
#endif
    }

    void balance_leaf(leaf_node *leaf, int max_level, 
                      keynode_vec_t &splits) {
        ...
        while (parent != nullptr && parent->level <= max_level) {
            if (splits.size() == 0) break;
            if (splits.size() == 1) {
                parent->slotkey[parent_slot] = splits[0].k;
                splits.clear();
...
#if 0
    std::pair<iterator, bool>
    insert_start_lazy2(const key_type& key, const data_type& value) {
        std::vector<key_type> newkeys;
        std::vector<node *> newnodes;

        BTREE_ASSERT(m_root);
       
        std::pair<iterator, bool> r = 
          insert_descend_lazy(m_root, key, value, newkeys, newnodes);
 
        // BTREE_ASSERT(newkeys.size() != 1);  // the max of root can be changed
        if (newkeys.size() > 1) {
            handle_root(newkeys, newnodes);
        }
    ...
    /**
     * @brief Insert an item into the B+ tree w/o balance.
     *
     * Descend down the nodes to a leaf, insert the key/data pair in a free
     * slot. If the node overflows, then it must be split and the new split
     * node inserted into the parent. Unroll / this splitting up to the root.
    */
    std::pair<iterator, bool> 
    insert_descend_lazy(node* n, const key_type& key, const data_type& value,
                        std::vector<key_type>& splitkeys, 
        ...
        if (!n->isleafnode()) 
        {
            ...
            std::pair<iterator, bool> r = 
                insert_descend_lazy(inner->childid[slot],
                                    key, value, splitkeys, splitnodes);

            if (splitkeys.size() == 1) {
                BTREE_ASSERT(slot == n->slotuse);
                inner->slotkey[slot] = splitkeys[0];
                splitkeys.clear();
                splitnodes.clear();
            } else if (splitkeys.size() > 1) {
                // insert inner node
                balance_inner(inner, slot, splitkeys, splitnodes);
            }
        ...
        else // n->isleafnode() == true
        {
            splitkeys.clear();
            splitnodes.clear();
            // TODO: vector capacity, multi_map?
            // BTREE_ASSERT(allow_duplicates);
            leaf_node *leaf = static_cast<leaf_node *>(n);
            lazy_insert(leaf, key, value);        

            // TODO: update lazy buffer
            if (leaf->is_lazy_full()) {
                update_lazy_buffer(leaf, splitkeys, splitnodes);
                m_stats.itemcount += lazyslotmax;
            }

            return std::pair<iterator, bool>(iterator(leaf, 0), true);
        } 
    }
#endif

    /// Insert lazy buffer
    inline void lazy_insert(leaf_node *leaf, const pair_type& value) {
        lazy_insert(leaf, value.first, value.second);
    }

    inline void lazy_insert(leaf_node *leaf, const key_type& key, const data_type& data) {
        int slot = leaf->lazy_buffer.slot_use++;
        if (slot == 0) {
            add_dirty_leaf(leaf);
        }

        std::vector<pair_type> &pairs = leaf->lazy_buffer.pairs;
        int sz = leaf->lazy_buffer.pairs_sz;
        if (slot == sz) {
            pairs.resize(sz * 2);
            leaf->lazy_buffer.pairs_sz = sz * 2;
        } else if (slot > leaf->lazy_buffer.pairs_sz) {
            while (slot > leaf->lazy_buffer.pairs_sz) ;
        }
        BTREE_ASSERT(slot < (int) pairs.size());
        leaf->lazy_buffer.pairs[slot].first = key;
        leaf->lazy_buffer.pairs[slot].second = data;
    }

    inline void balance_inner(inner_node *inner, unsigned short slot, 
                            keynode_vec_t& splits) {
        // the first splitnodes it inner it self
        int num_children = (inner->slotuse + 1) + (splits.size() - 1);
        int num_parents = inner_node::num_inner(num_children);
        keynode_vec_t new_splits(num_parents);

        int old_child_i = inner->slotuse;
        int new_i = splits.size() - 1;

        key_type max_key = inner->slotkey[old_child_i];

        // create new inner nodes
        for (int p = num_parents - 1; p >= 0; --p) {
            inner_node *n = (p == 0)? inner : allocate_inner(inner->level);
            n->slotuse = num_children / (p + 1);
            num_children -= n->slotuse;
            --n->slotuse; // an inner node has keys+1 children

            for (int c = n->slotuse; c >= 0; --c) {
                node *child = nullptr;
                key_type key;
                if (old_child_i > slot) {
                    child = inner->childid[old_child_i];
                    key = inner->slotkey[old_child_i];
                    --old_child_i;
                } else if (new_i >= 0) {
                    BTREE_ASSERT(old_child_i == slot);
                    child =  splits[new_i].n;
                    key = splits[new_i].k;
                    if (new_i == 0) --old_child_i;
                    --new_i;
                } else {
                    BTREE_ASSERT(old_child_i < slot && old_child_i >= 0);
                    if (p == 0) {
                      // BTREE_ASSERT(c == slot - 1); // false assert
                      break;
                    }
                ...
                n->slotkey[c] = key;
                n->childid[c] = child;
                child->set_parent(n, c);
            }

            new_splits[p] = { n->slotkey[n->slotuse], n };
        }

        BTREE_ASSERT(new_i == -1);

        // update vector
        if (num_parents == 1 && key_equal(inner->slotkey[inner->slotuse], max_key)) {
            splits.clear();
        } else {
            splits.assign(new_splits.begin(), new_splits.end());
        }
    }

    inline void add_dirty_leaf(leaf_node* leaf) {
        int slot = m_dirty_num++;
        if (slot == m_dirty_sz) {
            m_dirty_leaf.resize(m_dirty_sz * 2);
            m_dirty_sz *= 2;
        } else if (slot > m_dirty_sz) {
            while (slot > m_dirty_sz) ;
        }

        m_dirty_leaf[slot].first = leaf;
        m_dirty_leaf[slot].second = leaf->slotkey[leaf->slotuse - 1];  // max key
    }
    ...
    inline void update_lazy_buffer(leaf_node *leaf,
                                   keynode_vec_t &splits) {
        int num_items =  leaf->slotuse + leaf->lazy_buffer.slot_use;
        leaf->lazy_sort();
        int num_leaves = (num_items + leafslotmax - 1) / leafslotmax;
        int old_leaf_i = leaf->slotuse - 1;
        int buffer_i = leaf->lazy_buffer.slot_use - 1;
        splits.resize(num_leaves);
        key_type max_key = leaf->slotkey[old_leaf_i];

        // new leaves
        for (int i = num_leaves - 1; i >= 1; --i) {
            // allocate new leaf node
            leaf_node *n = allocate_leaf();

            // calculate slot of this new leaf
            n->slotuse = num_items / (i + 1);
            num_items -= n->slotuse;

            // merge key-value from biggest to least
            // TODO: use function like `key_greater`...
            for (int s = n->slotuse - 1; s >= 0; --s) {
                // old_leaf_i < 0 means insert into the head of this leafnode
                if (old_leaf_i < 0 || 
                    (buffer_i >= 0 && 
                      leaf->lazy_buffer.pairs[buffer_i].first >= leaf->slotkey[old_leaf_i])) {
                    n->set_slot(s, leaf->lazy_buffer.pairs[buffer_i]);
                    --buffer_i;
                } else {
                    n->set_slot(s, 
                        leaf->slotkey[old_leaf_i], leaf->slotdata[old_leaf_i]);
                    --old_leaf_i;
                }
            }
#if BTREE_COUNT == 1  // 1 for verifiy btree index
            {
              key_type minkey, maxkey;
              tree_stats vstats;
              vstats.level_nodes.resize(1, 0);
              verify_node(n, &minkey, &maxkey, vstats);
            }
#endif

            // double link
            if (i == num_leaves - 1) {
                n->nextleaf = leaf->nextleaf;
            } else {
                // i < num_leaves - 1
                leaf_node *next = static_cast<leaf_node *>(splits[i + 1].n);
                n->nextleaf = next;
                next->prevleaf = n;
            }

            // insert into vector
            splits[i] = { n->slotkey[n->slotuse - 1], n };
        }
        
        // this leaf
        leaf->slotuse = num_items;
        for (int s = leaf->slotuse - 1; s >= 0; --s) {
            if (buffer_i < 0) {
              BTREE_ASSERT(s == old_leaf_i);
              break;
            }

            // old_leaf_i < 0 means insert into the head of this leafnode
            if (old_leaf_i < 0 || leaf->lazy_buffer.pairs[buffer_i].first >= leaf->slotkey[old_leaf_i]) {
                leaf->set_slot(s, leaf->lazy_buffer.pairs[buffer_i]);
                --buffer_i;
            } else {
                BTREE_ASSERT(s != old_leaf_i && old_leaf_i >= 0);
                leaf->set_slot(s, 
                        leaf->slotkey[old_leaf_i], leaf->slotdata[old_leaf_i]);
                --old_leaf_i;
            }
...
#if BTREE_COUNT == 1  // 1 for verifiy btree index
        {
          ...
          tree_stats vstats;
          vstats.level_nodes.resize(1, 0);
          verify_node(leaf, &minkey, &maxkey, vstats);
        }
#endif

        // deal with header and tailer of double link list
        if (num_leaves > 1) {
            static_cast<leaf_node *>(splits[1].n)->prevleaf = leaf;
            if (leaf->nextleaf) {
                leaf->nextleaf->prevleaf = 
                  static_cast<leaf_node *>(splits[num_leaves - 1].n);
            } else {
                // leaf->next == nullptr
                if (leaf != m_tailleaf) {
                  printf("leaf %p, tail %p, tailnext %p\n", leaf, m_tailleaf, m_tailleaf->nextleaf);
                }
                BTREE_ASSERT(leaf == m_tailleaf);
                m_tailleaf = static_cast<leaf_node *>(splits[num_leaves - 1].n);
            }
            leaf->nextleaf = static_cast<leaf_node *>(splits[1].n);
            splits[0] = { leaf->slotkey[leaf->slotuse - 1], leaf };
        } else {
            if (leaf->slotkey[leaf->slotuse - 1] != max_key) {
                splits[0] = { leaf->slotkey[leaf->slotuse - 1], leaf };
                BTREE_ASSERT(leaf == m_tailleaf);
            } else {
                splits.clear();
    ...
    inline void handle_root(keynode_vec_t& splits) {
        int num_children = splits.size();
        BTREE_ASSERT(num_children > 1);
        int level = splits[0].n->level;
        int num_root = inner_node::num_inner(num_children);

        // the real root
        add_level(level + 1);

        if (num_root == 1) {
            inner_node *n = allocate_inner(level + 1);
            n->slotuse = num_children - 1;
            for (int i = 0; i < num_children; ++i) {
                n->slotkey[i] = splits[i].k;
                n->childid[i] = splits[i].n;
                splits[i].n->set_parent(n, i);
            }
            m_root = n;
        ...
        // recursive part
        int node_i = 0;
        for (int r = 0; r < num_root; ++r) {
            inner_node *n = allocate_inner(level + 1);
            n->slotuse = num_children / (num_root - r);
            num_children -= n->slotuse;
            --n->slotuse;

            for (int i = 0; i <= n->slotuse; ++i, ++node_i) {
                n->slotkey[i] = splits[node_i].k;
                n->childid[i] = splits[node_i].n;
                splits[node_i].n->set_parent(n, i);
            }

            // insert into vector
            splits[r] = { n->slotkey[n->slotuse], n };
        }
        splits.resize(num_root);
...
public:
    ...
    void bulk_load(Iterator ibegin, Iterator iend)
    {
        ...
        BTREE_ASSERT(m_stats.level_nodes[0] == num_leaves);

        // create first level of inner nodes, pointing to the leaves.
        size_t num_parents = inner_node::num_inner(num_leaves);
        ...
        for (int level = 2; num_parents != 1; ++level)
        {
            ...
            num_parents = inner_node::num_inner(num_children);
            ...
            for (size_t i = 0; i < num_parents; ++i)
            {
                ...
                for (unsigned short s = 0; s < n->slotuse; ++s)
                {
                    ...
                    nextlevel[inner_index].first->set_parent(n, s);
                    ++inner_index;
                }
                n->childid[n->slotuse] = nextlevel[inner_index].first;
                nextlevel[inner_index].first->set_parent(n, n->slotuse);
...
public:
    ...
    void verify() const
    {
        ...
        if (m_root)
        {
            int depth = m_root->level + 1;
            assert((int) m_stats.level_nodes.size() == depth);
            vstats.level_nodes.resize(depth, 0);
            ...
            for (int i = 0; i < depth; ++i) {
              assert(vstats.level_nodes[i] == m_stats.level_nodes[i]);
            }
...
private:
    ...
    void verify_node(const node* n, key_type* minkey, key_type* maxkey, tree_stats& vstats) const
    {
        ...
        if (n->isleafnode())
        {
            ...
            // assert(leaf->lazy_buffer.slot_use == 0);
        ...
        else // !n->isleafnode()
        {
            const inner_node* inner = static_cast<const inner_node*>(n);
            vstats.innernodes++;
            vstats.level_nodes[inner->level]++;
            ...
            for (unsigned short slot = 0; slot <= inner->slotuse; ++slot)
            {
                ...
                // parent check
                assert(subnode->parent == inner);
                assert(subnode->parent_slot == slot);

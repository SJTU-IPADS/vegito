...
public:
    ...
    inline void insert_lazy(const value_type& x) {
        tree.insert_lazy(x); 
    }

    inline void insert_lazy(const key_type &k, const data_type &d) {
        tree.insert_lazy(k, d);
    }
    ...
    // parallel balance (step by step)
    // 1. master
    inline void prepare_balance(int num_worker) {
        tree.prepare_balance(num_worker);
    }

    // 2. worker
    inline void parallel_balance(int worker_id) {
        tree.balance(worker_id);
    }

    // 3. master (include join)
    inline void end_balance() {
        tree.end_balance();
    }

    // parallel balance (call once)
    ...
    inline void prepare_leaf_node(void **hint_leaf) {
        tree.prepare_leaf_node(hint_leaf);
    }
    
    inline std::pair<iterator, bool> insert(const key_type& key, const data_type& data,
                                            void **hint_leaf)
    {
        return tree.insert2(key, data, hint_leaf);
    }

...
int ChLogCleaner::clean_log(int log_id, int partition_id, int tx_id,
  ...
  if (view.is_gp(log_id)) {
    if(config.isUseSegGraph()) {
      return clean_graph_log<livegraph::SegGraph>(graph_stores_[partition_id],
                                                  log_id, partition_id, tx_id,
                                                  table_id, key, seq, val,
                                                  length, op_bit, write_epoch);
    } else {
      return clean_graph_log<livegraph::Graph>(graph_stores_[partition_id],
                                                log_id, partition_id, tx_id,
                                                table_id, key, seq, val,
                                                length, op_bit, write_epoch);
    }
  ...
  if (config.getNumServers() == 1 && log_id == 1) {

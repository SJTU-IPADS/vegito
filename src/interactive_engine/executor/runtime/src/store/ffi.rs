...
impl GlobalGraphQuery for FFIGraphStore {
    ...
    fn get_vertex_properties(&self, si: i64, ids: Vec<(PartitionId, Vec<(Option<LabelId>, Vec<VertexId>)>)>, output_prop_ids: Option<&Vec<u32>>) -> Self::VI {
        ...
        let output_prop_ids = Arc::new(output_prop_ids.cloned());
        let iter = ids.into_iter().flat_map(move |(partition_id, ids)| {
            let output_prop_ids = output_prop_ids.clone();
            ids.into_iter().flat_map(move |(label, ids)| {
                if let Some(l) = label {
                    ...
                    FFIGetVertexIter::new(graph, iter, output_prop_ids.clone())
                } else {
    ...
    fn get_all_vertices(&self, si: i64, labels_ref: &Vec<LabelId>, condition: Option<&Condition>, dedup_prop_ids: Option<&Vec<u32>>, output_prop_ids: Option<&Vec<u32>>, limit: usize, partition_ids: &Vec<PartitionId>) -> Self::VI {
        ...
        let iter = partition_ids.clone().into_iter().map(|v| v as FFIPartitionId).flat_map(move |partition_id| {
            ...
            FFIGetAllVerticesIter::new(graph, iter, output_prop_ids.clone())
        });
        GlobalVertexIter::Normal(Box::new(iter))
    }
    ...
    fn count_all_vertices(&self, si: i64, labels_ref: &Vec<LabelId>, condition: Option<&Condition>, partition_ids: &Vec<PartitionId>) -> u64 {
        ...
        partition_ids.iter().foreach(|partition_id| {
            ...
            count += FFIGetAllVerticesIter::new(self.graph, iter, Arc::new(Some(vec![]))).count();
        });
        count as u64
    }
...
impl Vertex for GlobalVertex {
    type PI = FFIPartialPropertiesIter;
...
pub struct FFIVertex {
    graph: GraphHandle,
    handle: VertexHandle,
    output_prop_ids: Arc<Option<Vec<u32>>>,
}
...
impl Vertex for FFIVertex {
    ...
    fn get_properties(&self) -> Self::PI {
        if let Some(output_prop_ids) = &*self.output_prop_ids {
            FFIPartialPropertiesIter::new(self.graph, self.handle, self.output_prop_ids.clone())
        } else {
            FFIPartialPropertiesIter::empty(std::ptr::null(), 0)
        }
    }
...
impl FFIVertex {
    pub fn new(graph: GraphHandle, handle: VertexHandle, output_prop_ids: Arc<Option<Vec<u32>>>) -> Self {
        FFIVertex {
            graph: graph,
            handle: handle,
            output_prop_ids: output_prop_ids.clone(),
        }
    }
}
...
impl FFIGetVertexIter {
    pub fn new(graph: GraphHandle, iter: GetVertexIterator, output_prop_ids: Arc<Option<Vec<u32>>>) -> Self {
        FFIGetVertexIter {
            graph: graph,
            iter: iter,
            output_prop_ids: output_prop_ids.clone(),
...
impl Iterator for FFIGetVertexIter {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if state == STATE_SUCCESS {
            let v = FFIVertex::new(self.graph, vertex_handle, self.output_prop_ids.clone());
            return Some(v);
        }
        None
    }
...
impl FFIGetAllVerticesIter {
    pub fn new(graph: GraphHandle, iter: GetAllVerticesIterator, output_prop_ids: Arc<Option<Vec<u32>>>) -> Self {
...
pub struct FFIPartialPropertiesIter {
    graph: GraphHandle,
    handle: VertexHandle,
    output_prop_ids: Arc<Option<Vec<u32>>>,
    current_index: usize,
...
impl FFIPartialPropertiesIter {
    pub fn new(graph: GraphHandle, handle: VertexHandle, output_prop_ids: Arc<Option<Vec<u32>>>) -> Self {
        FFIPartialPropertiesIter {
            graph: graph,
            handle: handle,
            output_prop_ids: output_prop_ids.clone(),
            current_index: 0,
    ...
    pub fn empty(graph: GraphHandle, handle: VertexHandle) -> Self {
        FFIPartialPropertiesIter {
            graph: graph,
            handle: handle,
            output_prop_ids: Arc::new(Some(Vec::new())),
            current_index: 0,
        }
    }
}

impl Iterator for FFIPartialPropertiesIter {
    type Item = (PropId, Property);

    fn next(&mut self) -> Option<Self::Item> {
        if let Some(output_prop_ids) = &*self.output_prop_ids {
            if self.current_index < output_prop_ids.len() {
                let prop_id = output_prop_ids[self.current_index];
                self.current_index += 1;

                let mut property = NativeProperty::default();
                let state = unsafe { get_vertex_property(self.graph, self.handle, prop_id as PropertyId, &mut property) };
                if state == STATE_SUCCESS {
                    if let Some(property) = property.to_property() {
                        return Some((prop_id, property));
                    }
                }
            }
        }
...
impl Schema for FFISchema {
    ...
    fn get_prop_name(&self, prop_id: u32) -> Option<String> {
        ...
        // dbg!("get prop name: ", &prop_id, &state, STATE_SUCCESS);
        if state == STATE_SUCCESS {
            let c_name = unsafe { CStr::from_ptr(name) };
            let ret = c_name.to_owned().into_string().ok();
            // dbg!("get prop name returns", &c_name, &ret);

...
class DBTX : public TXHandler {
 ...
 public:
  ...
  DBTX(MemDB *tables, int t_id,Rpc *rpc,int c_id = 0, LogTSManager *tsm = NULL);
  DBTX(MemDB *db);
  ...
  bool stream_end(yield_func_t &yield);
  ...
  void write(int tableid,uint64_t key,char *val,int len,uint32_t op);
  void write();
  ...
  void remote_write(int r_idx,char *val,int len, uint32_t op);

  /* streamming operation (stateless) */
  void stream_insert(int tableid,uint64_t key,const char *val,int len); 

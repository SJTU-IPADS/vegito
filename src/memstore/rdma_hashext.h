...
// using google::dense_hash_map;      // namespace where class lives by default
    ...
    public:
      ...
      RdmaHash(int len,char* arr){

        //entrysize = (((esize-1)>>3)+1) <<3;
        entrysize = sizeof(Node);
        // round up
      ...
      virtual uint64_t RemoteTraverse(uint64_t key,rdmaio::Qp *qp) {
        ...
        // send an read
        fetch_node(qp,loc,exch,sizeof(HeaderNode));

        HeaderNode* node = (HeaderNode*) exch;
        ...
        while (true) {
          for (int i = 0; i < CLUSTER_H; i++) {
            if (node->keys[i] == key && node->indexes[i] != 0) {
              ...
              loc += sizeof(DataNode);
              fetch_node(qp,loc + base_off,exch,sizeof(Node));
              Node *node = (Node *)(exch);
              auto res = node->off;
              Rfree(exch);
#if CACHING == 1
      ...
      virtual Node* Put(uint64_t k, uint64_t* val) {
        return _GetWithInsert(k,(char *)val);
      }

      virtual Node *_GetWithInsert(uint64_t key,char *val) {
          ...
          if (node->next != 0 ) {
            ...
            for(uint i = 0;i < CLUSTER_H;++i) {
              if(node->indexes[i] == 0) {
                ...
                node->indexes[i] = free_data;
                free_data++;
                res = (Node *)((char *)free_node + sizeof(DataNode));
...
typedef RdmaHash<MemNode, Memstore> RdmaHashExt;

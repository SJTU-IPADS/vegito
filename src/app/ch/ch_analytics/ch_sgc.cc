...
void ChAnalyticsWorker::process_sgc_init() {
  
}

bool ChAnalyticsWorker::process_sgc() {
  ...
  for(int idx = 0; idx < ctx.vertex_labels.size(); idx++) {
    ...
    if(col_cursor.get()) {
      std::cout << "[SGC] Use col cursor" << std::endl;
      col_cursor->seekOffset(0, vertices_num);
      col_cursor->nextRow();
      for (int i = 0; i < vertices_num; i++) {
        auto cursor_data = col_cursor->value();
        for(int d = 0; d < ctx.params.input_dim; d++) {
          ctx.params.raw_feature[idx][i](d) = *(double*)(cursor_data + d*sizeof(double));
        }
        col_cursor->nextRow();
      }
    } else {
      std::cout << "[SGC] Use row cursor" << std::endl;
      assert(row_cursor);
      ...
      for (int i = 0; i < vertices_num; i++) {
        ...
        for(int d = 0; d < ctx.params.input_dim; d++) {
          ctx.params.raw_feature[idx][i](d) = *(double*)(data + d*sizeof(double));
        }
        row_cursor->nextRow();
      }
    }
...
bool ChAnalyticsWorker::grape_sgc() {
  // UNINPLEMENTED
  return true;
}

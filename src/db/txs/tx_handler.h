...
namespace nocc {
  namespace db {
    ...
    protected:
      ...
      uint64_t gossip_cnt_;

    public:
      
      RemoteSet *remoteset;
      uint     cor_id_;
      TXHandler(int c_id) : cor_id_(c_id), gossip_cnt_(0) { }
      
      uint64_t get_gossip_cnt() const { return gossip_cnt_; }
    
      void begin(DBLogger *db_logger = NULL, TXProfile *p = NULL) {
        return _begin(db_logger, p);
      }

      virtual void begin(int tx_id, DBLogger *db_logger) {
        assert(false);
      }

      virtual void local_ro_begin() = 0;
      virtual bool end(yield_func_t &yield) = 0; /* like [commit] function  */
      virtual bool stream_end(yield_func_t &yield) {
        assert(false);
      ...
      /* Modifiers */
      virtual void write(int tableid,uint64_t key,char *val,int len,uint32_t op) = 0 ;
      ...
      virtual void stream_insert(int tableid,uint64_t key,const char *val,int len) { 
        assert(false);
      }
      ...
      virtual void remote_write(int idx,char *val,int len, uint32_t op) = 0;

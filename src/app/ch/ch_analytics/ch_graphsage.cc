...
#include "app/analytics/graphsage.hpp"
...
void ChAnalyticsWorker::process_graphsage_init() {
  
}

bool ChAnalyticsWorker::process_graphsage() {
  ...
  GraphSageCtx ctx(num_thread_, graph_store_, rg_map_, edge_labels, read_ver);  
  ...
  ctx.params.weight_1 = MatrixXd::Random(2*ctx.params.hidden_dim, ctx.params.output_dim);
  ...
  for(int idx = 0; idx < ctx.vertex_labels.size(); idx++) {
    ...
    if(col_cursor.get()) {
      std::cout << "[GraphSage] Use col cursor" << std::endl;
      ...
      std::cout << "[GraphSage] Use row cursor" << std::endl;
      assert(row_cursor);
  ...
  // work
  parallel_process<GraphSageCtx>(ctx, run_graphsage);

  std::cout << "Finish GraphSage, "
...
bool ChAnalyticsWorker::grape_graphsage() {
  // UNINPLEMENTED
  return true;
}

...
def copy_files(exe,config_file,mac_set):
    ...
    for host in mac_set:
        subprocess.call(["scp",config_file,"%s:%s" % (host,"~")],stdout=FNULL, stderr=subprocess.STDOUT)
        subprocess.call(["scp",exe,"%s:%s" % (host,"~")],stdout=FNULL, stderr=subprocess.STDOUT)
    return
...
def init_checks(config,arg):
    ...
    try:
        ...
        ## check whether there are remaining processes of running
            return check_process_status(mac_set)
    except:
...
def calculate_results(log_file_name):
    ...
    for line in f:
        ...
        c += 1
    if (c < END_EPOCH):
        print_with_tag("res","error, counting: %d" % c)
        return (0,0,default_lats,0)
    c = END_EPOCH - WARM_EPOCH
    #c -= WARM_EPOCH
    thr = thr / float(c)
    aborts = aborts / float(c)
    abort_ratio = abort_ratio / float(c)
    ...
    f.close()
    return (thr / 1000.0,1.0 - aborts,abort_ratio,lats)
                    ...
                    for s in settings:
                        ...
                        while (True):
                            ...
                            try: ## check whether results exists
                                ...
                                results[log_file_name] = [0.0,0.0,0.0,[0,0,0]]

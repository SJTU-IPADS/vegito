...
size_t MemstoreBPlusTree::sizeLeaf(LeafNode *n) {
  return sizeof(*n);
}
	
size_t MemstoreBPlusTree::sizeInner(InnerNode *n, unsigned depth) {
  size_t res = 0;
  for (int i=0; i<=n->num_keys; i++)
    if (depth>1) 
      res += sizeInner(reinterpret_cast<InnerNode*>(n->children[i]), depth-1);
    else 
      res += sizeLeaf(reinterpret_cast<LeafNode*>(n->children[i]));
  res += sizeof(*n);
  return res;
}

size_t MemstoreBPlusTree::sizeStore() {
  if(root == NULL) {
    return 0;
  }
  printf("Tree depth %d\n", depth);
  if (depth == 0) 
    return sizeLeaf(reinterpret_cast<LeafNode*>(root));
  else 
    return sizeInner(reinterpret_cast<InnerNode*>(root), depth);
} 

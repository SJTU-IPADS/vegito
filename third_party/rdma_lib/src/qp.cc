...
#endif
	send_cq = ibv_create_cq(rdma_device->ctx, RC_MAX_SEND_SIZE, NULL, NULL, 0);
	if(send_cq == NULL) {
		fprintf(stderr,"[librdma] qp: Failed to create cq, %s\n", strerror(errno));
	}
	assert(send_cq != NULL);

	static __thread struct ibv_cq * r_cq = NULL;
	if(r_cq == NULL) {
		r_cq = ibv_create_cq(rdma_device->ctx, RC_MAX_SHARED_RECV_SIZE, NULL, NULL, 0);
		if(r_cq == NULL) {
			fprintf(stderr,"[librdma] qp: Failed to create cq, %s\n", strerror(errno));
		}
	}

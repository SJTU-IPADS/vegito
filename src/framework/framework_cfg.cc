...
void Config::parse_sys_xml() {
  ...
  try{
    num_primaries_ = pt.get<int>("bench.num_primary");
    assert(num_primaries_ <= num_servers_); 
  } catch (const ptree_error &e) {
    num_primaries_ = num_servers_;
  }
  ...
  try {
    rep_gp_factor_ = pt.get<int>("bench.rep_gp_factor");
  } catch (const ptree_error &e) {
  ...
  try {
    analytics_nthreads_ = pt.get<size_t>("bench.analytics_threads");
  } catch (const ptree_error &e) {
  ...
  try {
    ana_session_ = pt.get<int>("bench.analytics_session");
  } catch (const ptree_error &e) {
    ana_session_ = 1;
  }
  ...
  // ******** Graph **********
  try {
    use_grape_engine_ = pt.get<bool>("bench.graph.grape_engine");
  } catch (const ptree_error &e) {
    use_grape_engine_ = true;
  }
  
  try {
    use_seg_graph_ = pt.get<bool>("bench.graph.seg_graph");
  } catch (const ptree_error &e) {
    use_seg_graph_ = false;
  }
  ...
  try {
    a_start_sec_ = pt.get<int>("bench.a_start_sec"); 
  } catch (const ptree_error &e) {
    a_start_sec_ = 0;
  }
  ...
  try {
    a_round_ = pt.get<int>("bench.a_round"); 
  } catch (const ptree_error &e) {
    a_round_ = 100;
  }
...
void Config::printConfig() const {
  ...
  printf("    3) #analytics threads: %d\n", analytics_nthreads_);
  printf("    4) #backup threads: %d\n", backup_nthreads_);
  printf("    5) #slave corroutines in each thread: %d\n", num_routines_);
  printf("  6. Log\n");
  ...
  printf("  9. Graph\n");
  printf("    1) use seg-CSR?: %d\n", use_seg_graph_);
  printf("    2) use Graph engine?: %d\n", use_grape_engine_);
  printf(" 10. Client\n");
  ...
  printf("    5) analytics start sec: %d\n", a_start_sec_);
  printf("    6) analytics round: %d\n", a_round_);
  printf("    7) cleaner start sec: %d\n", cleaner_start_sec_);

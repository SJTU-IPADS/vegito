...
LogWorker::LogWorker(unsigned worker_id, bool fake)
...
void LogWorker::run() {
  ...
  while(true) {
    ...
    for (int l_id = 0; l_id < num_logs_; ++l_id) {
      ...
      bool is_gp = view.is_gp(l_id);
      if ((is_ap || is_gp) && need_barrier_) {
        barrier_(barrier_e_);
        continue;
      }
      ...
      for(int t_id = start_tid; t_id < final_tid; t_id += thread_gap) {
        ...
        for(int n_id = 0; n_id < num_nodes_; ++n_id) {
          if ((is_ap || is_gp) && need_barrier_) break;
          ...
          if (tailer_ptr == NULL) {
            ...
            //        n_id, t_id, l_id);
            if ((is_ap || is_gp) && config.isUseEpoch() && config.getEpochType() != 1) {
...
backup_clean_log(int n_id, int t_id, int l_id, char *poll_ptr,
  ...
  if (!config.isUseEpoch() || !(is_ap || is_gp)) {

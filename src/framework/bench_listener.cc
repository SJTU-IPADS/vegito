...
Listener::Listener(const std::vector<BenchWorker *> &workers,
  ...
  :epoch_(0),
   ...
   txn_stopped_(false), qry_started_(false), log_started_(false)
{
  assert(cm != NULL);
  assert(total_partition < 64);   // for ret_bitmap_
  ...
  if(LOG_RESULTS && current_partition == 0) {
    ...
    snprintf(log_file_name, 64, "./results/%s_%s_%lu_%lu_%lu_%lu.log",
             ...
             total_partition,nthreads,coroutine_num,distributed_ratio);

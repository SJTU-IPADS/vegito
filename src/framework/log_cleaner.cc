...
LogCleaner::LogCleaner(bool version) 
  : need_version_(version),
    ...
    backup_map2_(config.getNumPrimaries(), nullptr),
    graph_stores_(config.getNumPrimaries(), nullptr),
    mappings_(config.getNumPrimaries(), nullptr)   
  { }  // no bug here
...
void LogCleaner::add_graph_store(GraphStore *graph, RGMapping *mapping) {
  int p_id = mapping->getPID();
  assert(p_id < config.getNumPrimaries());
  // TODO: for single-machine

  graph_stores_[p_id] = graph;
  mappings_[p_id] = mapping;
}

...
struct GraphSageCtx : public nocc::oltp::AnalyticsCtx {
  GraphSageParam params;

  GraphSageCtx(int num_workers,
...
void run_graphsage(int worker_idx, void *arg) {
  bool is_master_worker = (worker_idx == 0);
  auto ctx = static_cast<GraphSageCtx*>(arg);
  ...
  // apply layer 0
  nocc::oltp::process_vertices<GraphSageCtx, int>(
      worker_idx, ctx,
  ...
  // gather layer 0
  nocc::oltp::process_edges_pull<GraphSageCtx, int>(worker_idx, ctx,
      [&](label_t dst_vlabel, label_t src_vlabel, vertex_t dst, EdgeIteratorBase& incoming_adj) {
          auto msg = VectorXd::Zero(params.hidden_dim);
          while(incoming_adj.valid()) {
              vertex_t src = incoming_adj.dst_id();
              msg.cwiseMax(trans_0[src_vlabel][src]);
              incoming_adj.next();
          }
          // concat
          hidden_result[dst_vlabel][dst] = VectorXd(params.hidden_dim*2);
          hidden_result[dst_vlabel][dst] << trans_0[dst_vlabel][dst], msg;
          return 0;
      },
      active
  );
  ...
  nocc::oltp::process_edges_pull<GraphSageCtx, int>(worker_idx, ctx,
      [&](label_t dst_vlabel, label_t src_vlabel, vertex_t dst, EdgeIteratorBase& incoming_adj) {
          auto msg = VectorXd::Zero(params.output_dim);
          while(incoming_adj.valid()) {
              vertex_t src = incoming_adj.dst_id();
              msg.cwiseMax(trans_1[src_vlabel][src]);
              incoming_adj.next();
          }
          // concat
          result[dst_vlabel][dst] = VectorXd(params.output_dim*2);
          result[dst_vlabel][dst] << msg, msg;
          return 0;
      },
      active
  );

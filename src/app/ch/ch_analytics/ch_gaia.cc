...
void ChAnalyticsWorker::process_gaia_init() {
   std::cout<< "Init gaia ######"<<std::endl;
   initialize_pegasus(1);
}

bool ChAnalyticsWorker::process_gaia() {
  ...
  auto vlabel_idx = ctx.vlabel2idx[CUST];
  for (int i = 0; i < ctx.vertex_nums[vlabel_idx]; i++) {
    auto edge_iter = ctx.seg_graph_readers[vlabel_idx]->get_edges(i, C_I, EOUT);
    if (edge_iter.size() > 0) {
      int64_t label_id = CUST;
      int64_t global_id = i;
  ...
  */  
  ...
  ic9_timer.start();
  evaluate_query_plan("/disk1/wanglei/htap/src/gremlin-queries/tpcc/tpcc_plans/IS_3.plan" /* which ldbc query to evaluate */, 1);
...
bool ChAnalyticsWorker::grape_gaia() {
  return process_gaia();
}

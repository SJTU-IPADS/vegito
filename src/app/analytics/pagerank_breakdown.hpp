...
void run_pagerank(int worker_idx, void *arg) {
  ...
  // 1. init rank value
  nocc::util::Breakdown_Timer timer_topo_1;
  timer_topo_1.start();

  double delta_1 = nocc::oltp::process_edges_push<PageRankCtx, double>(
      worker_idx,
      ctx,
  ...
  barrier->wait();
  float topo_time_1_sec = timer_topo_1.get_diff_ms()/1000;

  nocc::util::Breakdown_Timer timer_all_1;
  timer_all_1.start();
  ...
  float all_time_1_sec = timer_all_1.get_diff_ms()/1000;

  // only topo
  nocc::util::Breakdown_Timer timer_topo_2;
  timer_topo_2.start();
  ...
  float topo_time_2_sec = timer_topo_2.get_diff_ms()/1000;

  // 2. comp&topo
  nocc::util::Breakdown_Timer timer_all_2;
  timer_all_2.start();
  ...
  barrier->wait();
  float all_time_2_sec = timer_all_2.get_diff_ms()/1000;

  std::cout << " Get topo time: " << topo_time_1_sec + topo_time_2_sec << " seconds "<< std::endl;
  std::cout << " Get comp time: " << 
   (all_time_1_sec + all_time_2_sec)- (topo_time_1_sec + topo_time_2_sec) << " seconds" << std::endl;
}

...
class ChRunner : public Runner {
 public:
  ...
  virtual vector<BenchLoader *>
      make_backup_loaders(int partition, BackupDB *store);
  virtual vector<BenchWorker *> make_workers();
  virtual vector<BenchClient *> make_clients();
  virtual vector<LogWorker *> make_backup_workers();
  virtual vector<QueryWorker *> make_qry_workers();
...
void ChRunner::init_backup_store(BackupDB &store) {
  ...
  // 5 order
  {
    ...
    schema.cols = 
        { 
#if 1
          { sizeof(oo.o_carrier_id), true, PAGE_SIZE / sizeof(oo.o_carrier_id) },
#else
          { sizeof(oo.o_carrier_id), true, 1 },
#endif
          { sizeof(oo.o_c_id), false },
          { sizeof(oo.o_ol_cnt), false },
          { sizeof(oo.o_all_local), false },
#if 1
          { sizeof(oo.o_entry_d), false, 0 } 
#else
          { sizeof(oo.o_entry_d), false, 0, BINDEX_BTREE } 
#endif
        };
    schema.max_items = TimeScale;
    schema.hash_fn = orde_hash_fn;
  ...
  // 6 order-line
  {
    ...
    schema.cols = 
        { 
#if 1
          { sizeof(ol.ol_delivery_d), true, PAGE_SIZE / sizeof(ol.ol_delivery_d) },
#else
          { sizeof(ol.ol_delivery_d), true, uint64_t(-1) },
#endif
          { sizeof(ol.ol_i_id), false },
          { sizeof(ol.ol_amount), false },
          { sizeof(ol.ol_supply_w_id), false },
          { sizeof(ol.ol_quantity), false } };
...
vector<BenchWorker *> ChRunner::make_workers() {
  ...
  if (rest_worker == 0) return vector<BenchWorker *>();
...
vector<QueryWorker *> ChRunner::make_qry_workers() {
  ...
  if (backups.size() == 0 || num_qry == 0 || num_session == 0) return ret;
...
vector<LogWorker *> ChRunner::make_backup_workers() {
  ...
  if (backup_stores_.size() == 0 || backup_nthreads == 0)
    return ret;
